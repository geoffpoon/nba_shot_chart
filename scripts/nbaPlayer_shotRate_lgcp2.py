...
full_DatFrame = pd.read_csv('../dat/joined_shots_2014.csv')
...
def plot_player_normLambda(player):
    norm_lambdaN_v = np.loadtxt('player2014_lambda_seed%d/norm_lambda_%s.txt'%(player))
...
directory = 'player2014_lambda_seed%d'%(randSeed)
...
for i, player in enumerate(top_players_nameList):
    try:
        norm_lambdaN_v = np.loadtxt('player2014_lambda_seed%d/norm_lambda_%s.txt'%(randSeed, player))
    except:
        ...
        np.savetxt('player2014_lambda_seed%d/norm_lambda_%s.txt'%(randSeed, player), norm_lambdaN_v)
    print(player)
    LL[i,:] = norm_lambdaN_v[:]
